...
test("реплика для задачи включает варианты ответа", () => {
  const task = diagnostic[0];
  const speech = speechForTask(task);
  assert.match(speech, /Варианты ответа/);
  for (const option of task.options) assert.ok(speech.includes(option));
});
