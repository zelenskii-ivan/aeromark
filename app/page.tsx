...
const LESSON_TITLE: Record<"diagnostic" | "mistakes" | "practice" | "lesson", string> = {
  diagnostic: "Проверочный полёт",
  mistakes: "Работа над ошибками",
  practice: "Бесконечная тренировка",
  // Режим урока рисует собственный экран и сюда не попадает.
  lesson: "Урок",
};
...
export default function Home() {
  ...
  const continueTitle = !saved.diagnosticDone
    ? "Проверочный полёт"
    : nextLesson
      ? nextLesson.title
      : gameUnlocked && !gameComplete
        ? "Игровой эпизод в парке"
        : nextMission
          ? nextMission.title
  ...
  /**
   * Урок ведёт статистику навыков так же, как задания, но звёзд за отдельный
   * шаг не даёт: звезда выдаётся за пройденный урок целиком, иначе ребёнок
   * гонится за счётчиком вместо понимания.
   */
  const answerLessonStep = (step: Step, correct: boolean) => {
    const skill = stepSkill(step);
    if (!skill || !isInteractive(step)) return;
    setSaved((current) => ({
      ...current,
      ...
      attempts: {
        ...current.attempts,
        [skill]: {
          right: (current.attempts[skill]?.right ?? 0) + (correct ? 1 : 0),
          total: (current.attempts[skill]?.total ?? 0) + 1,
        },
      },
    }));
  };
  ...
  if (
    ...
    (sync.state.phase === "signed-in" && !sync.state.childId)
  ) {
    return (
          ...
          <TabsContent value="missions" className="tab-content">
            ...
            <div className="section-heading">
              <div>
                ...
                <h2>Новое разбираем по шагам</h2>
              </div>
              <span>
                {saved.completedLessons.length} из {lessons.length} пройдено
              </span>
            </div>
            <div className="lesson-grid">
              {lessons.map((item) => {
                const done = saved.completedLessons.includes(item.id);
                return (
                  <button
                    key={item.id}
                    type="button"
                  ...
                  >
                    ...
                    <p>{item.goal}</p>
                    <footer>
                      <span>{item.steps.length} шагов</span>
                      <ChevronRight />
                    </footer>
                  </button>
                );
